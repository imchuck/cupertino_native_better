  ...
  init(frame: CGRect, viewId: Int64, args: Any?, messenger: FlutterBinaryMessenger) {
    ...
    if split && count > rightCount {
      ...
      DispatchQueue.main.async { [weak self, weak left, weak right] in
        ...
        // El hueco se mide DESPUÉS del layout y en otro ciclo: la
        // vista de fondo que dibuja la píldora se coloca en el pase
...
channel.setMethodCallHandler { [weak self] call, result in
  ...
  /// El rectángulo que de verdad se DIBUJA de una barra, en coordenadas del
  /// contenedor.
  ///
  /// **No es su marco.** Medido en un iPhone 17 Pro: una `UITabBar` no llena su
  /// marco — dibuja su píldora de Liquid Glass ajustada a su contenido —, así
  /// que el hueco entre marcos y el hueco entre píldoras son cosas distintas.
  /// Con cuatro items y dos rótulos por lado difieren 14,7 pt. Quien pinta algo
  /// en ese hueco necesita el segundo, no el primero.
  private func rectanguloDibujado(_ bar: UITabBar) -> CGRect {
    for v in bar.subviews {
      let nombre = NSStringFromClass(type(of: v))
      if nombre.contains("BarBackground") || nombre.contains("Background") {
        return bar.convert(v.frame, to: container)
      }
    }
    // Si el fondo no aparece —otra versión de iOS, otra jerarquía— se devuelve
    // el marco. Es peor, pero es una degradación y no un cero.
    return bar.convert(bar.bounds, to: container)
  }

  /// Avisa a Dart de dónde cae el hueco entre las dos mitades, MEDIDO.
  ///
  /// El lado nativo solo devolvía el ancho total, así que quien quisiera pintar
  /// un botón central tenía que deducir la geometría del layout — y la
  /// deducción falla en cuanto UIKit cambia dónde dibuja la píldora dentro de
  ...
  private func reportarHueco(_ left: UITabBar, _ right: UITabBar) {
    ...
    guard ancho.isFinite, ancho > 0, container.bounds.width > 0 else { return }
    channel.invokeMethod("splitGap", arguments: [
